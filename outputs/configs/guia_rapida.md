...
| `citus/scripts/init.sql` | DDL + extensión citus + índices |
| `citus/scripts/load_data.py` | Carga del dataset en Citus |

## Comandos de ejecución (orden)

```bash
# 1. Limpieza del dataset (IA genera, humano ejecuta)
...
python mongodb/scripts/load_data.py

# 5. Levantar Citus cluster
cd citus && docker compose up -d
# Esperar ~20s

# 6. Verificar workers Citus
docker exec citus-coordinator psql -U postgres -d news_analysis_pg -c "SELECT * FROM master_get_active_worker_nodes()"
